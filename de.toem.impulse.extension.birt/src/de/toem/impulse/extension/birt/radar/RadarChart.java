...
public class RadarChart extends AbstractBirtChartCell {
    ...
    public Chart createBirtChart(ITheme theme) {
        ...
        for (int s = 1; s < dataSets.keySet().size(); s++) {
            ...
            // colors & fonts
            seRadar.getLabel().getCaption().setColor(textColor.copyInstance());
            seRadar.getLabel().setShadowColor(scaleColor.copyInstance());
            seRadar.getCatLabel().getCaption().setColor(textColor.copyInstance());
            seRadar.getCatLabel().setShadowColor(scaleColor.copyInstance());  
            seRadar.getWebLineAttributes().setColor(scaleColor.copyInstance());  
            seRadar.getLineAttributes().setColor(scaleColor.copyInstance());  
