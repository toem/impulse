...
int main() {
	...
	unsigned char memoryBuffer[bufferSize * 2]; // for 2 buffers
	unsigned char memoryTrace[traceSize];

	// buffers
	flxBuffer buffer2 = flxCreateLinearBuffer(memoryBuffer + bufferSize * 1, bufferSize, flxWriteToFile, out); // writes to files
	flxBuffer buffer1 = flxCreateLinearBuffer(memoryBuffer + bufferSize * 0, bufferSize, flxCompressLz4, buffer2); // compresses to buffer 2

	// trace - start with buffer 2 (no compression)
	flxTrace trace = flxCreateTrace(0, MAX_ITEM_ID, MAX_ENTRY_SIZE, memoryTrace, traceSize, buffer2);
