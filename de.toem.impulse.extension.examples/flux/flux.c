...
// attachments
#define FLX_ENTRY_ATAS  0x40 /* Association */
...
flxresult flxWriteAssociationEntry(flxBuffer buffer, flxid itemId,
...
flxresult flxWriteAssociation(flxTrace trace, flxid itemId, flxuint target,
	...
	if (!flxIsOpen(trace, itemId))
		return FLX_ERROR_NOT_OPEN;
// write value
	return flxWriteAssociationEntry(trace->buffer, itemId, target, style, delta);
}
