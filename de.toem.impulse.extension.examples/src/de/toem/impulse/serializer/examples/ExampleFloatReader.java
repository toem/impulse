...
import de.toem.impulse.samples.IStructSample;
...
public class ExampleFloatReader extends AbstractSingleDomainRecordReader {
    ...
    protected void parse(IProgress progress, InputStream in) throws ParseException {
        ...
        Signal float1 = addSignal(system, "Signal1", "My first float Signal (limitet to 32 bits)", ProcessType.Discrete, SignalType.Float, SignalDescriptor.Float32);
        ...
        Signal float3 = addSignal(another, "Signal3", "Another float (limitet to 32 bits)", ProcessType.Discrete, SignalType.Float, SignalDescriptor.Float64);
        Signal float4 = addSignal(another, "Signal4", "Rectangle", ProcessType.Discrete, SignalType.Float, SignalDescriptor.DEFAULT);
        Signal float5 = addSignal(another, "Signal5", "XY", ProcessType.Discrete, SignalType.FloatArray, new SignalDescriptor(SignalDescriptor.CONTENT_DEFAULT,2,ISample.FLOAT_ACCURACY_64,ISample.FORMAT_DEFAULT));
