...
import de.toem.impulse.cells.configuration.FolderConfiguration;
import de.toem.impulse.cells.configuration.RecordConfiguration;
import de.toem.impulse.cells.configuration.SamplesConfiguration;
import de.toem.impulse.cells.record.Scope;
import de.toem.impulse.cells.record.Signal;
import de.toem.impulse.domain.TimeBase;
import de.toem.impulse.other.examples.CreateConfiguration;
...
public class ExampleIntegerReader extends AbstractSingleDomainRecordReader {
    ...
    protected void parse(IProgress progress, InputStream in) throws ParseException {
        ...
        RecordConfiguration config = addConfiguration("myconfig1", null);
        FolderConfiguration folder = addFolderConfiguration(config, "eventFolder", null);
        SamplesConfiguration samples = addSamplesConfiguration(folder, int1);
        samples = addSamplesConfiguration(folder, int2);
        samples.columnValueFormat = ISample.FORMAT_HEXADECIMAL;

        // a second one
        config = addConfiguration("myconfig2", null);
        ...
        try {
            ...
            if (wallet != null)
                config = addConfigurations(wallet);
        } catch (IOException e) {
        }
