package de.toem.impulse.producer.examples;
...
public class ExampleFloatDiff extends AbstractSamplesProducer {
    ...
    static public IPropertyModel getPropertyModel() {
        return new PropertyModel().add("multiplier", "1.0", "Multiplier");
    }

    @Override
    public void produce(List<IReadableSamples> sources) {
        ...
        // parameters
        double multiplier = Utils.parseDouble(parameters.getVal("multiplier"),1.0);
        ...
        if (inputs.size() == 2 && signalType == SignalType.Float && processType == ProcessType.Discrete) {
            ...
            reference = PackedSamples.createReader(out, readerBase);
        }
    }
