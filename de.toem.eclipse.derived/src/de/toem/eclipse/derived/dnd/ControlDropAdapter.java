...
/**
 * This adapter class provides generic drag-and-drop support for a control. Derived from ViewerControlAdapter
 * <p>
 ...
 * </ul>
 * The <code>setFeedbackEnabled</code> method can be called to turn on and off visual insertion feedback (on by default).
 * </p>
 */
public abstract class ControlDropAdapter extends DropTargetAdapter {
	...
	/**
	 * The last valid operation. We need to remember the last good operation in the case where the current operation temporarily is not valid (drag over
	 * someplace you can't drop).
	 */
	private int lastValidOperation;

	/**
	 * This is used because we allow the operation to be temporarily overridden (for example a move to a copy) for a drop that happens immediately after the
	...
	/**
	 * Information about the position of the mouse relative to the target (before, on, or after the target. Location is one of the <code>LOCATION_* </code>
	 * constants defined in this type.
	 */
	private int currentLocation;

	/**
	 * A flag that allows adapter users to turn the insertion feedback on or off. Default is <code>true</code>.
	 */
	private boolean feedbackEnabled = true;

	/**
	 * A flag that allows adapter users to turn auto scrolling on or off. Default is <code>true</code>.
	 */
	private boolean scrollEnabled = true;

	/**
	 * A flag that allows adapter users to turn auto expanding on or off. Default is <code>true</code>.
	 */
	private boolean expandEnabled = true;

	/**
	 * A flag that allows adapter users to turn selection feedback on or off. Default is <code>true</code>.
	 */
	 ...
	 *            the viewer
	 */
	protected ControlDropAdapter() {
	}

	protected abstract Control getControl();

	/**
	 * Clears internal state of this drop adapter. This method can be called when no DnD operation is underway, to clear internal state from previous drop
	 * operations.
	 * 
	 * @since 3.5
	 */
...
//	    Utils.log(event.widget,event.item);
		if (!(event.item instanceof Item)) {
			if (getControl() instanceof Tree || getControl() instanceof Table)
				return determinePostLocation(event);
			return LOCATION_NONE;
		}
		Item item = (Item) event.item;
		Point coordinates = new Point(event.x, event.y);
		coordinates = getControl().toControl(coordinates);
		if (item != null) {
			Rectangle bounds = getBounds(item);
			if (bounds == null) {
				return LOCATION_NONE;
			}
//			Utils.log(coordinates.y ,bounds.y, bounds.height);
			if ((coordinates.y - bounds.y) < 5) {
				
				return LOCATION_BEFORE;
			}
			if ((bounds.y + bounds.height - coordinates.y) < 5) {
				// System.out.println("after"+bounds.y +" "+ bounds.height +" "+ coordinates.y);
	...
	/**
	 * Returns the position of the given event when the area below the latest element is targeted.
	 * 
	 * @param event
	 *            the event
	 * @return one of the <code>LOCATION_* </code>constants defined in this class
	 */
	protected int determinePostLocation(DropTargetEvent event) {
		return LOCATION_NONE;
	}
	...
	protected Object determineTarget(DropTargetEvent event) {
		...
		if (getControl() instanceof Tree || getControl() instanceof Table) {
			return determinePostTarget(event);
		}
		return null;
	}

	/**
	 * Returns the target item of the given drop event when the area below the latest element is targeted.
	 * 
	 * @param event
	 *            the event
	 * @return The target of the drop, may be <code>null</code>.
	 */
	protected Object determinePostTarget(DropTargetEvent event) {
		return null;
	}

	/*
	 * (non-Javadoc) Method declared on DropTargetAdapter. The mouse has moved over the drop target. If the target item has changed, notify the action and check
	...
	private void doDropValidation(DropTargetEvent event) {
		...
		// System.out.println("doDropValidation");
		// System.out.println("" + event.detail + " " + lastValidOperation + " " + overrideOperation);
		// System.out.println(currentOperation == DND.DROP_COPY ? "DROP_COPY" : currentOperation == DND.DROP_MOVE ? "DROP_MOVE"
		// : currentOperation == DND.DROP_LINK ? "DROP_LINK" : "DROP_NONE");
		// System.out.println(getCurrentLocation() == LOCATION_BEFORE ? "LOCATION_BEFORE" : getCurrentLocation() == LOCATION_AFTER ? "LOCATION_AFTER"
		// : getCurrentLocation() == LOCATION_ON ? "LOCATION_ON" : "NONE");
		...
		if (currentOperation == DND.DROP_NONE) {
			if (validateDrop(currentTarget, DND.DROP_MOVE, event.currentDataType))
				currentOperation = DND.DROP_MOVE;
			else if (validateDrop(currentTarget, DND.DROP_LINK, event.currentDataType))
				currentOperation = DND.DROP_LINK;
			else
				currentOperation = DND.DROP_NONE;
		}
	...
	/*
	 * (non-Javadoc) Method declared on DropTargetAdapter. The drag has entered this widget's region. See if the drop should be allowed.
	 */
	...
	/*
	 * (non-Javadoc) Method declared on DropTargetAdapter. The drop operation has changed, see if the action should still be enabled.
	 */
...
//		System.out.println(target + " " + currentLocation);
	...
	/*
	 * (non-Javadoc) Method declared on DropTargetAdapter. The user has dropped something on the desktop viewer.
	 */
	...
	/*
	 * (non-Javadoc) Method declared on DropTargetAdapter. Last chance for the action to disable itself
	 */
	 ...
	 * 
	 * This may be called only inside of the {@link #validateDrop(Object, int, TransferData)} or {@link #performDrop(Object)} methods.
	 * 
	 ...
	 * 
	 * @return the selected object or the first element in current selection, and null if no objects are selected
	 */
	protected abstract Object getSelectedObject();

	/**
	 * @deprecated this method should not be used. Exception handling has been removed from DropTargetAdapter methods overridden by this class. Handles any
	 *             exception that occurs during callback, including rethrowing behavior.
	 *             <p>
	 *             [Issue: Implementation prints stack trace and eats exception to avoid crashing VA/J. Consider conditionalizing the implementation to do one
	 *             thing in VAJ and something more reasonable in other operating environments. ]
	 *             </p>
	 * 
	 * @param exception
	 *            the exception
	 ...
	 * @param data
	 *            the drop data
	 * @return <code>true</code> if the drop was successful, and <code>false</code> otherwise
	 */
	 ...
	 * 
	 * This maybe called only from within a {@link #validateDrop(Object, int, TransferData)} method
	 * 
	 * 
	 * @param operation
	 ...
	 * 
	 * @param value
	 *            <code>true</code> if visual feedback is desired, and <code>false</code> if not
	 */
	 ...
	 * 
	 * @param value
	 *            <code>true</code> if selection feedback is desired, and <code>false</code> if not
	 * 
	 * @since 3.2
	 */
	 ...
	 * 
	 * @param value
	 *            <code>true</code> if scrolling and expanding is desired, and <code>false</code> if not
	 * @since 2.0
	 */
	 ...
	 * 
	 * @param value
	 *            <code>true</code> if expanding is desired, and <code>false</code> if not
	 * @since 3.4
	 */
	 ...
	 * 
	 * @param value
	 *            <code>true</code> if scrolling is desired, and <code>false</code> if not
	 * @since 3.4
	 */
	 ...
	 * 
	 * @param target
	 *            the object that the mouse is currently hovering over, or <code>null</code> if the mouse is hovering over empty space
	 * @param operation
	 *            the current drag operation (copy, move, etc.)
	 * @param transferType
	 *            the current transfer type
	 * @return <code>true</code> if the drop is valid, and <code>false</code> otherwise
	 */
